...
std::optional<Instruction> Arm64Decoder::disasm(const uint8_t* Bytes, uint64_t Size, uint64_t Addr)
{
    cs_insn* Instruction;
...
std::optional<Instruction> Arm64Decoder::build(const cs_insn& CsInstruction)
{
...
std::optional<Operand> Arm64Decoder::build(const cs_arm64_op& CsOp)
{
    // FIXME:
    auto str_toupper = [](std::string s) {
        std::transform(s.begin(), s.end(), s.begin(),
