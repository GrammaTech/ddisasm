...
bool Table::parseLine(const std::string& x)
{
	...
	std::cerr << "[FAILURE] Could not parse output line: \"" << x << "\".  Read " << row.size() << " columns, expected " << this->columns << "." << std::endl;
	return false;
}
...
{
	auto found = std::find_if(std::begin(this->table), std::end(this->table), 
		[&key](auto element)
		{
