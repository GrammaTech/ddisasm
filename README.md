...
| elfSymbolVersions       | `std::tuple<ElfSymDefs,ElfSymNeeded,ElfSymbolVersionsEntries>` |      Tuple with symbol version definitions, needed symbol versions, and a mapping from symbols to symbol version identifier and hidden attribute `ElfSymbolVersionsEntries = std::map<gtirb::UUID, std::tuple<SymbolVersionId,bool>>`. Symbol version definitions are  `ElfSymDefs = std::map<SymbolVersionId, std::vector<std::string>>`, a map from symbol version identifiers to the list of versions strings. The first element of the list is the version itself, the subsequent elements are predecessor versions. The needed symbol versions are `ElfSymNeeded = std::map<std::string, std::map<SymbolVersionId, std::string>>`, a map from dynamic library names to the symbol versions that they need. For each library, we have a map from version identifiers to version strings. Symbol version identifiers are `SymbolVersionId = uint16_t` integers.                                                                                                                                                                             |
