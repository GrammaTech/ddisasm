...
| ddisasmVersion          | `std::string`                                                                                      | The version of ddisasm used to produce the GTIRB.                                                                                                                                                                      |
...
| elfSectionIndex         | `std::map<uint64_t, gtirb::UUID>`                                                                  | Map from ELF section indices to section UUIDs.                                                                                                                                                                         |
| elfSymbolInfo           | `std::map<gtirb::UUID, std::tuple<uint64_t, std::string, std::string, std::string, uint64_t>>      | ELF Symbol information: Size, Type, Binding, Visibility, and SectionIndex.                                                                                                                                             |
| elfSymbolTabIdxInfo     | `std::map<gtirb::UUID, std::vector<std::tuple<std::string, uint64_t>>>`                            | ELF Symbol section information: The name of the symbol table where it was declared and the index within that table.                                                                                                    |
| elfSymbolVersions       | `std::map<gtirb::UUID, std::string>`                                                               | ELF Symbol versions: The version of each symbol.
