...
def compile(*args):
    proc = subprocess.run(
        ["cl32", "/nologo", *args], stdout=subprocess.DEVNULL
    )
    ...
    def bdrt(self, name, *sources):
        ...
        def inner(*flags):
            with cd(examples_path / name):
                ...
                self.assertTrue(
                    disassemble(
                        "ex.exe", False, extension="asm", extra_args=["-F"]
                    )
                )
    ...
    def test_ex_legacy_switch_001(self):
        def inner(*flags):
            with cd(examples_path / "ex_legacy_switch.001"):
                ...
                self.assertTrue(
                    disassemble(
                        "main.exe", False, extension="asm", extra_args=["-F"]
                    )
                )
                ...
                for char in string.ascii_uppercase:
                    self.assertTrue(
                        diff_output(["main.exe", char], ["out.exe", char])
                    )
                cleanup()
        ...
        def inner(*flags):
            with cd(examples_path / "ex_legacy_switch.003"):
                ...
                for arg in [750, 800, 900, 700, 500, 250, 100, 200, 600]:
                    self.assertTrue(
                        diff_output(
                            ["main.exe", str(arg)], ["out.exe", str(arg)]
                        )
                    )
                cleanup()
